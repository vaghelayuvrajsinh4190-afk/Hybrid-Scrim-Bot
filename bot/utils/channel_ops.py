...
async def ensure_text_channel(
    ...
    *field_key* is the dot-path suffix inside ``channel_ids``
    ...
    stored_id = (panel or {}).get("channel_ids", {}).get(field_key)
    ...
    for ch in guild.text_channels:
        if ch.name.lower() == name.lower() and ch.category_id == category.id:
            await panels_col().update_one(
                {"guild_id": guild_id, "panel_id": panel_id},
...
async def ensure_role(
    ...
    """Return (or create) the tag-access role for a panel."""
    panel = await panels_col().find_one(
        {"guild_id": guild_id, "panel_id": panel_id}
    )
    stored_id = (panel or {}).get("role_id")
